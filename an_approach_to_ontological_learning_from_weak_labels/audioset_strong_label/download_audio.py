...
def format_audio(input_audio_file, output_audio_file):
    ...
    cmdstring = "ffmpeg -loglevel panic -i %s -ac 1 -ar 44100 %s" % (
        input_audio_file, temp_audio_file)
    os.system(cmdstring)
    cmdstring1 = "sox %s -G -b 16 -r 44100 %s" % (
...
def trim_audio(input_audio_file, output_audio_file, start_time, duration):
    ...
    cmdstring = "sox %s %s trim %s %s" % (
        input_audio_file, output_audio_file, start_time, duration)
    os.system(cmdstring)
...
def download_audio_method(line, csv_file):
    ...
    query_id = splited_line[0]
    # query_id, start_seconds, end_seconds = line.split("\t")
    start_seconds = splited_line[1]
    end_seconds = splited_line[2]
    # import pdb
    # pdb.set_trace()
    audio_duration = float(end_seconds) - float(start_seconds)
    # positive_labels = ','.join(line.split(",")[3:]);
    sufix = query_id.split('_')[-1]
    query_id = query_id[:-len(sufix) - 1]
    ...
    try:
        ...
        output_folder = sys.argv[1].split('.csv')[0].split(
            "/")[-1] + "_" + csv_file.split('.csv')[0] + "_" + "audio_downloaded"
        # print output_folder
        ...
        formatted_folder = sys.argv[1].split('.csv')[0].split(
            "/")[-1] + "_" + csv_file.split('.csv')[0] + "_" + "audio_formatted_downloaded"
        ...
        # Trimming code
        segmented_folder = sys.argv[1].split('.csv')[0].split(
            "/")[-1] + "_" + csv_file.split('.csv')[0] + "_" + "audio_formatted_and_segmented_downloads"
        if not os.path.exists(segmented_folder):
            os.makedirs(segmented_folder)
        path_to_segmented_audio = segmented_folder + "/Y" + \
            query_id + '_' + start_seconds + '_' + end_seconds + ".wav"
        trim_audio(path_to_formatted_audio, path_to_segmented_audio,
                   start_seconds, audio_duration)
